...
func DefaultRedisManagerConfig(conn string) *ManagerConfig  {
	return &ManagerConfig{CookieName:"minipcapisid",Gclifetime:30*24*3600,EnableSetCookie:true,ProviderConfig:conn, CookieLifeTime:0, Domain:".eastday.com"} //sessionid 默认存30天
}
...
func (manager *Manager) SessionStart(w http.ResponseWriter, r *http.Request) (session Store, err error) {
	...
	if manager.config.CookieLifeTime > 0 {
		...
		//remmberme
		remmberme := r.URL.Query().Get("remmberme")
		if "1" == remmberme {
			cookielifetime := 30*24*3600
			cookie.MaxAge = cookielifetime
			cookie.Expires = time.Now().Add(time.Duration(cookielifetime) * time.Second)
		}
	}
