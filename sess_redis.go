...
func (rp *RedisProvider) SessionRead(sid string) (Store, error) {
	c := rp.poollist.Get()
	defer c.Close()
...
func (rp *RedisProvider) SessionRegenerate(oldsid, sid string) (Store, error) {
	c := rp.poollist.Get()
	defer c.Close()
